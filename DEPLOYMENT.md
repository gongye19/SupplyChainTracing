...
3. **INSIGHT_JOBS_ENABLED** - Insight 任务总开关
   - Worker 部署前保持 `false`
   - Worker 验证完成后设置为 `true`

4. **DEFAULT_DATASET_VERSION** - 默认数据发布版本，例如 `2026-07-01`

5. **INSIGHT_WORKER_TOKEN** - Railway 与服务器 Worker 共用的高强度随机密钥
   - 只保存在 Railway 和服务器环境变量中
   - 不得配置到 Vercel 或前端代码
...
**Q: 数据加载失败**
A: 
1. 检查 `supplychain-data-pipeline` 最近一次发布是否成功
2. 检查后端日志查看具体错误
3. 检查 API 端点是否正确
